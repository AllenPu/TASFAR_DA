        ...
        for row_index, row in cal_house.iterrows():
            ...
            if temp_di == DOMAIN_INDEX_DICT[domain_index]:
                ...
                if self.pseudo_label_path:
                    ...
                    temp_list.append(np.array([self.pseudo[str_index][0]]))
                    temp_list.append(np.array([self.pseudo[str_index][1]]))
                    temp_list.append(np.array([self.pseudo[str_index][2]]))
                    temp_list.append(np.array([self.gmd]))
                self.p_index += 1
